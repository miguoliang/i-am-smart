...
**内容**是可替换的文件：`peilian-pack/v1`。

场景：上课时打开首页日历，点当天，把这节课的**词汇**和**问答**记下来。一课一份，课后拿出来练。
...
上课日规则存在本机，不写在这一节课的 JSON 里。导出某一节课时会带上 `courseId` / `scheduledOn`。
...
- **上课记**：首页日历点某一天。没有这天的课会先建一节空课。词汇回车记下；有空格的当成问答，答句可补
- **排课**：首页 → 更多 → 排课 → 选每周几、排出几周（可选，日历上会出现空心点）
- **巩固**：首页 → 巩固，或点已记的课。选一节课或已学全部，再定词汇和问答的数量
- **完整过一遍**：点已记的课 → 刚记完的第一遍
- **编辑**：已记的课 → 编辑
- **复制示例**：示例课 → 复制并编辑
- **导入备份**：首页 → 更多 → 导入备份
...
自定义的课和排期存在本机 IndexedDB，换浏览器或清站点数据会丢失，请用导出备份或云端同步。
