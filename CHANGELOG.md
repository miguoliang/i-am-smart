...
- **分享学习进度图**：导出卡片与内嵌进度块、二维码外框均为**直角**，不再带圆角；全屏预览去掉按宽度比例裁剪圆角，仅保留阴影与自适应尺寸
- **Learn 单词卡**：未揭晓时背面文案用 `invisible` 占位保持高度，覆盖层改为简洁 **skeleton**（`animate-pulse`），去掉渐变与说明文案

### 发布

- 版本号 **v1.12.22**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.22`**
...
- **分享学习进度图预览**：全屏预览中图片随高度/宽度缩放时，用与导出卡片相同的半径比例（`cqw`）裁剪圆角，避免固定 `rounded-2xl` 与 PNG 内圆角错位

### 发布

- 版本号 **v1.12.21**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.21`**
...
- **Learn 单词卡**：未揭晓时译文遮罩区增加渐变、占位条与「释义 · 选择下方评分后显示」提示，避免纯空白难以感知下方仍有释义

### 发布

- 版本号 **v1.12.20**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.20`**
...
- **Learn 单词卡**：译文区域始终占位，未揭晓时用与卡片同色的遮罩盖住释义，揭晓时仅去掉遮罩，**避免**点击后卡片高度突然变化造成的视觉跳动

### 发布

- 版本号 **v1.12.19**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.19`**
...
- **Learn 页背景**：纹理与配色仅按档案 **考试目标**（`exam_target`）解析；未设置时默认 KET；**不再**用 CEFR 单词级别（A1–C2）作为背景水印或色板来源

### 发布

- 版本号 **v1.12.18**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.18`**
...
- **分享学习进度图**：页顶改**单行左右分栏**——左侧品牌（过长省略），右侧 **学习进度 · 词库名**；名言区上边距略减

### 发布

- 版本号 **v1.12.17**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.17`**
...
- **分享学习进度图**：压缩页顶信息——**工具名 · 当前词库** 同一行，**学习进度** 单独一行；去掉原「当前词库」独占行与「我的学习进度」长标题；名言区略收紧边距与字号

### 发布

- 版本号 **v1.12.16**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.16`**
...
- **分享学习进度图**：**点击后**先请求最新词库进度（「正在获取进度…」），再加载出图依赖并生成 PNG（「正在生成图片…」）；分享数据不再依赖打开设置时预取的进度；成功后回写 React Query 缓存以便与考试列表进度条一致

### 发布

- 版本号 **v1.12.15**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.15`**
...
- **分享学习进度图**：**点击分享后**再动态加载 `html-to-image` 与 `qrcode` 并生成图片，避免打开设置即加载出图相关代码；按钮在出图时显示「正在生成图片…」与加载图标，不再用词库进度加载文案造成「提前生成图」的误解

### 发布

- 版本号 **v1.12.14**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.14`**
...
- **分享学习进度图**：去掉「学习档案」以节省版面；标题下展示与学习相关的**名人名言**（内置多条，每次生成随机一条）及出处

### 发布

- 版本号 **v1.12.13**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.13`**
...
- **分享学习进度图**：优化圆角与描边（外层 `overflow` 裁切、`box-shadow` 替代 `border` 减轻截图毛边；内外圆角层级与预览 `rounded-2xl` 对齐）

### 发布

- 版本号 **v1.12.12**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.12`**
...
- **分享学习进度**：入口移至学习页 **设置**（「分享」区块）；分享图为当前档案与考试目标下的 **词库进度**（已学 / 总词、约覆盖比例）与落地页二维码，不再分享单张单词卡

### 发布

- 版本号 **v1.12.11**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.11`**
...
- **刷词分享图**：修复部分环境（尤其 WebKit / 微信内置浏览器）导出**空白图**——改用 `left` 移出视口替代 `transform`、卡片纯色背景、截图前等待布局与二维码图片解码，并调整 `toPng` 参数

### 发布

- 版本号 **v1.12.10**（`NEXT_PUBLIC_APP_VERSION` 与 `apps/pwa`、仓库根 `package.json` 一致）；Git tag **`v1.12.10`**
