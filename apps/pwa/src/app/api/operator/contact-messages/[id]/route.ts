...
/** PATCH: Update contact message status and/or operator note */
export async function PATCH(
  ...
  try {
    ...
    if (!id) {
      throw ApiError.validationError('缺少留言 ID');
    }
    ...
    if (error) {
      logger.error('Operator contact message PATCH failed', { message: error.message });
      throw ApiError.internal(PUBLIC_INTERNAL_ERROR_MESSAGE);
    }
    if (!data) {
      throw ApiError.notFound('留言不存在');
    }
